...
use qp_plonky2_core::CircuitConfig;
...
use crate::plonk::circuit_data::{
    CircuitData, CommonCircuitData, ProverCircuitData, ProverOnlyCircuitData, VerifierCircuitData,
...
pub trait Read {
    ...
    fn read_target_bool_vec(&mut self) -> IoResult<Vec<BoolTarget>> {
        ...
        for _ in 0..length {
            result.push(self.read_target_bool()?);
        }
        Ok(result)
    }
    ...
    fn read_target_ext_vec<const D: usize>(&mut self) -> IoResult<Vec<ExtensionTarget<D>>> {
        ...
        let mut result = try_with_capacity(length)?;
        for _ in 0..length {
            result.push(self.read_target_ext::<D>()?);
        }
        Ok(result)
    }
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        // CONSTANTS_SIGMAS: num_constants + num_routed_wires (no salt)
        ...
        // WIRES: num_wires + salt
        let wires_v = self.read_field_vec(config.num_wires + salt)?;
        let wires_p = self.read_merkle_proof()?;
        evals_proofs.push((wires_v, wires_p));

        // ZS_PARTIAL_PRODUCTS: (num_challenges * (1 + num_partial_products) + num_all_lookup_polys) + salt
        let zs_partial_count = config.num_challenges * (1 + common_data.num_partial_products)
            + common_data.num_all_lookup_polys();
        let zs_partial_v = self.read_field_vec(zs_partial_count + salt)?;
        let zs_partial_p = self.read_merkle_proof()?;
        evals_proofs.push((zs_partial_v, zs_partial_p));

        // QUOTIENT: quotient_degree_factor * num_challenges + salt
        let quotient_v =
            self.read_field_vec(common_data.quotient_degree_factor * config.num_challenges + salt)?;
    ...
    fn read_target_fri_query_rounds<const D: usize>(
        ...
        for _ in 0..num_query_rounds {
            ...
            let mut steps = try_with_capacity(num_steps)?;
            for _ in 0..num_steps {
                steps.push(self.read_target_fri_query_step::<D>()?);
            }
    ...
    fn read_target_fri_proof<const D: usize>(&mut self) -> IoResult<FriProofTarget<D>> {
        let length = self.read_usize()?;
        let mut commit_phase_merkle_caps = try_with_capacity(length)?;
        for _ in 0..length {
